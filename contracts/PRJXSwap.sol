...
contract PRJXSwap is Context, Ownable {
    using SafeMath for uint256;
    using Address for address;

    IUniswapV2Router02 public uniswapRouter = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    IUniswapV2Pair public uniswapPair = IUniswapV2Pair(0x4342d13560c7E8Cd716E83e450F7432DF693680E);
    IERC20 public token;
    ...
    function quote(
        ...
        if(amount0 != 0) {
            (uint amountB) = uniswapRouter.quote(amount0, reserve0, reserve1);
            finalAmount = amountB;
        }

        if(amount1 != 0) {
            (uint amountB) = uniswapRouter.quote(amount1, reserve1, reserve0);
            finalAmount = amountB;
        }
    ...
    event UpdateUniswaps(address indexed router, address indexed pair);
    function updateUniswaps(
        address router, 
        address pair
    ) external onlyOwner {
        uniswapRouter = IUniswapV2Router02(router);
        uniswapPair = IUniswapV2Pair(pair);
        emit UpdateUniswaps(router, pair);
    }
